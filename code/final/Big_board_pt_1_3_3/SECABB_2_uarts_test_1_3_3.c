 ...
 *              !!!Modified scheduler!!!
 *              AUX uart loopback test
 *    -- Wire RB13 (U1RX) to RB7 (U1TX) for AUX machine UART1 loopback
 *       Runs up to 115200 baud, and probably 250000
 *    -- Wire RPA1  (U2RX) to USB TX line for console
 *    -- Wire RPB10 (U2TX) to USB RX line for console
 * 
 * Author:      Bruce Land
...
// while UART2 communicates with the console as usual
...
static PT_THREAD (protothread_serial(struct pt *pt))
{
    PT_BEGIN(pt);
      ...
      PT_terminate_char = '\r' ; // see  http://www.asciitable.com/
      ...
      while(1) {  
            ...
            // === command parser ================================
             switch(cmd[0]){
                 ...
                 case 't': // set terminators on main UART console DMA receive function
                ...
                case 'm':
                    ...
                    // the parameters set using the 't' command will be active 
                    // in machine input mode
                    if (mode == 1) {
                        mode=2;
                ...
                case 's':
                    ...
                    // e.g. the command s 123# puts 123# into the PT_send_buffer_aux
                    sscanf(PT_term_buffer, "%s %s", cmd, PT_send_buffer_aux);
                    // 
                    ...
                    // test for AUX channel time out otherwise
                    // copy the data
                     if (PT_timeout_aux == 1){
                ...
                case 'b':
                    // send a burst of trasmissions by AUX uart to loopback
                    // read the number of transmissions 
                    // terminator character is '#'
                    sscanf(PT_term_buffer, "%s %d", cmd, &v1);
                    
                    for (i=0; i<v1; i++){                       
                        PT_YIELD_UNTIL(pt, ready_to_send==1);
                        // compute the string payload
                        sprintf(PT_send_buffer_aux, "%d#", time_tick_millsec);
                        ready_to_receive = 1;
                        ready_to_send = 0 ;                       
                        PT_YIELD(pt); // let receive thread set up
                        // send using AUX USART to loopback
                        PT_SPAWN(pt, &pt_DMA_output_aux, PT_DMA_PutSerialBuffer_aux(&pt_DMA_output_aux) );
                        // check for timeout
                        PT_YIELD_UNTIL(pt, ready_to_send==1);
                         if (PT_timeout_aux == 1){
...
static PT_THREAD (protothread_serial_aux(struct pt *pt))
{
    PT_BEGIN(pt);
      ...
      PT_terminate_char_aux = '#' ; // see  http://www.asciitable.com/
      PT_terminate_count_aux = max_chars_aux ;
      // time in milliseconds!
      PT_terminate_time_aux = 1000 ;
